  ...
  const submenu = () => {
    return (
      <ul className="bg-white border border-gray-300 rounded-md shadow-lg mt-2 p-2">
        <li className="p-2 hover:bg-gray-200 cursor-pointer rounded-md"><a href='/mybooking'>My Booking</a></li>
  ...
  return (
    <>
      <nav className={` sticky top-0 scroll-smooth z-10 flex items-center justify-between  shadow-lg  h-[4rem] lg:px-[10rem] app-container ${theme==='light'?'bg-themeLight text-gray-600':'bg-gray-950'}`}>
        ...
        {mobilemenu && (
          <div
            className={`absolute flex flex-col top-[4rem]  left-0    h-[calc(100vh-4rem)] bg-cyan-400  transition ease-in-out duration-1000  overflow-y-auto  ${
              mobilemenu ? "w-64" : " w-0"
            }`}
          >
            <div className="flex-grow ">
              ...
              <ul className="p-4 ">
                ...
                <li className="hover:bg-gray-100 hover:text-primary tracking-wider font-serif text-gray-600 px-4 py-2 rounded-md truncate">
                  <NavLink
                    to="/mybooking"
                    className={({ isActive }) =>`block truncate ${isActive ? "text-primary" : ""}`
